    ...
    if (hosts.length === 0) {
      throw new Error(
        `Nothing to scan in ${range}: every host is a blocked target — loopback, ` +
          `link-local/cloud-metadata, multicast, or a reserved/documentation range ` +
          `(e.g. 192.0.2.0/24 is TEST-NET, not a real subnet). Point the scan at your ` +
          `actual LAN subnet (RFC1918, e.g. 192.168.x.0/24 or 10.x.0.0/24).`
      );
    }
