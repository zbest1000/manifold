...
| `MANIFOLD_BLOCK_PRIVATE_TARGETS` | `0` | Set to `1` to block the scanner / outbound clients from RFC1918/LAN targets (hardening for internet-exposed instances). By default LAN targets are allowed — Discovery and on-prem i3X/CESMII need them — while loopback and cloud-metadata link-local are **always** blocked |
